...
(function(){
  ...
  function reportInner(printMode=false){
    ...
    const rows = x.largest.length ? x.largest.map(h=>'<tr><td>'+safe(n(h.magnitude)?n(h.magnitude).toFixed(2)+'”':'—')+'</td><td>'+safe(areaName(h))+'</td><td>'+safe(countyName(h))+'</td><td>'+safe(fmtDate(h.event_date))+'</td><td><b>'+safe(qualityLabel(h))+'</b><br><small>'+safe(sourceLabel(h))+'</small></td></tr>').join('') : '<tr><td colspan="5">No hail reports for this filter.</td></tr>';
  ...
  const st=document.createElement('style'); st.textContent='.mfr-hail-report-modal{align-items:flex-start!important;overflow:auto!important;padding:24px!important}.mfr-hail-report-shell{width:min(1120px,96vw);margin:auto;background:#f3f6fb;border-radius:26px;padding:16px;box-shadow:0 24px 80px rgba(15,23,42,.25)}.mfr-hail-report-toolbar{display:flex;justify-content:flex-end;gap:8px;margin-bottom:8px}.mfr-hail-report-shell .mfr-hail-print-doc{padding:0}.mfr-hail-side-card{max-height:calc(100vh - 210px)!important;display:flex!important;flex-direction:column!important}.mfr-hail-list{overflow:auto!important;min-height:0!important;padding-right:4px}.mfr-hail-event-card.confirmed{border-left:4px solid #16a34a}.mfr-hail-event-card.radar{border-left:4px solid #f59e0b}.mfr-hail-pill.ok{background:#dcfce7!important;color:#166534!important}.mfr-hail-pill.warn{background:#fef3c7!important;color:#92400e!important}@media(max-width:860px){.mfr-hail-side-card{max-height:none!important}.mfr-hail-report-modal{padding:8px!important}.mfr-hail-report-shell{width:100%;padding:10px;border-radius:18px}}'; document.head.appendChild(st);
})();
